...
```
aws dynamodb get-item --table-name events --key '{"email":{"S":"email@example.com"}}'
```

Get specific fields from an item

```
aws dynamodb get-item --table-name events --key '{"email":{"S":"email@example.com"}}' --attributes-to-get event_type
```

Delete item using key

```
aws dynamodb delete-item --table-name events --key '{"email":{"S":"email@domain.com"}}'
```
...
```
aws s3 cp s3://bucket-name/backup.tar .
# Output example:
