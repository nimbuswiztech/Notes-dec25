...
{% stepper %}
{% step %}
### Web, DB, SSH security groups

* Web Server Security Group: Allow HTTP (port 80) and HTTPS (port 443) from anywhere
* Database Security Group: Allow MySQL (port 3306) only from web server security group
* SSH Access Group: Allow SSH (port 22) only from specific IP ranges
{% endstep %}
{% endstepper %}
